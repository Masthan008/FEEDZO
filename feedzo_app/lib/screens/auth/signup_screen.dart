...
class _SignupScreenState extends State<SignupScreen> {
  ...
  Future<void> _signup() async {
    ...
    if (!mounted) return;
    if (ok) {
      // Pop all screens — _AppRoot will show MainShell since isLoggedIn is now true
      Navigator.of(context).popUntil((route) => route.isFirst);
    } else {
      setState(() => _error = context.read<AuthProvider>().errorMessage ?? 'Sign up failed. Try again.');
    }
  }
        ...
        child: SingleChildScrollView(
          ...
          child: Form(
            ...
            child: Column(
              ...
              children: [
                const Text('Create Account', style: TextStyle(fontSize: 24, fontWeight: FontWeight.bold, color: AppColors.textPrimary)),
                const SizedBox(height: 4),
                const Text('Join Feedzo today', style: TextStyle(color: AppColors.textSecondary)),
                ...
                Row(
                  ...
                  children: [
                    ...
                    GestureDetector(onTap: () => Navigator.pop(context), child: const Text('Sign In', style: TextStyle(color: AppColors.primary, fontWeight: FontWeight.w600))),
                  ],
                ),
              ],
            ),
